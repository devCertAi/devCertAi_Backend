...
function parseSender() {
  const raw = process.env.MAIL_FROM || 'DevCert <no-reply@devcert.app>'
  const match = raw.match(/^"?([^"<]*)"?\s*<(.+)>$/)
  if (match) {
    return { name: match[1].trim() || 'DevCert', email: match[2].trim() }
  }
  return { name: 'DevCert', email: raw.trim() }
}

async function sendEmail({ to, subject, html }, attempt = 1) {
  if (!process.env.BREVO_API_KEY) {
    console.warn(`[Email] BREVO_API_KEY not set — skipping send. Would have sent "${subject}" to ${to}.`)
    return
  }
  ...
  try {
    ...
    if (status && [400, 401, 403].includes(status)) {
      console.error(
        `[Email] Brevo API rejected the send to ${to} (HTTP ${status}): ` +
        `${brevoError ? JSON.stringify(brevoError) : err.message}. ` +
        `Check BREVO_API_KEY is a valid API key (not the SMTP key) and that the MAIL_FROM sender/domain is verified in Brevo.`
      )
      throw err
    }

    if (attempt < MAX_SEND_ATTEMPTS) {
      console.warn(`[Email] Send to ${to} failed (attempt ${attempt}/${MAX_SEND_ATTEMPTS}): ${brevoError ? JSON.stringify(brevoError) : err.message}. Retrying...`)
      await new Promise((resolve) => setTimeout(resolve, 1500 * attempt))
      return sendEmail({ to, subject, html }, attempt + 1)
    }

    console.error(`[Email] Giving up sending to ${to} after ${MAX_SEND_ATTEMPTS} attempts: ${brevoError ? JSON.stringify(brevoError) : err.message}`)
    throw err
  }
}
